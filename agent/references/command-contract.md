...
- Use `pire-browser profiles` before asking the user for Firefox profile paths. It lists discovered Firefox sources, imported sources, and preserved 0.2.x profiles. `--profile <name>` copies the source without volatile caches into a temporary snapshot; it never mutates or continuously syncs the source. `--profile <path>` deliberately uses that path as a durable profile. If the source profile is active, ask the user to close Firefox before retrying; stale lock artifacts without a matching Firefox process do not block snapshots. Use `profiles usage`, `profiles clean`, and `profiles delete` for explicit legacy recovery; never delete profile directories by inference.
