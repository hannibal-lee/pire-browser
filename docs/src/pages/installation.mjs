...
pire-browser snapshot`),
  ...
  p("Use this when Pi should load the packaged extension and skill. After install, ask Pi to use <code>pire-browser</code> for browser automation. The extension scopes browser commands to the current Pi session and closes its managed Firefox session when Pi quits, reloads, forks, or switches sessions. Sessions explicitly targeted with another <code>--session</code> value remain caller-owned."),
